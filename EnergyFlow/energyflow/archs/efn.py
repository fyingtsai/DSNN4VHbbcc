...
from tensorflow.keras.layers import Concatenate, Dense, Dot, Dropout, Input, Lambda, TimeDistributed, Add, add
from tensorflow.keras.models import Model
from tensorflow.keras.regularizers import l2

from EnergyFlow.energyflow.archs.archbase import NNBase, _get_act_layer
from EnergyFlow.energyflow.archs.dnn import construct_dense
from EnergyFlow.energyflow.utils import iter_or_rep
...
class SymmetricPerParticleNN(NNBase):
    ...
    def _construct_Phi(self):
        ...
        ''''
        tmp_tensor = self.tensors[-1]
        Phi_layers, Phi_tensors = [], [self.tensors[-1]]
        d_layer1 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer1')
        dense_layer1 = TimeDistributed(d_layer1, name='d_layer1')
        act_layer1 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer1, act_layer1])
        Phi_tensors.append(dense_layer1(Phi_tensors[0]))
        Phi_tensors.append(act_layer1(Phi_tensors[0]))

        d_layer2 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer2')
        dense_layer2 = TimeDistributed(d_layer2, name='d_layer2')
        act_layer2 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer2, act_layer2])
        Phi_tensors.append(dense_layer2(dense_layer1(Phi_tensors[0])))
        Phi_tensors.append(act_layer2(dense_layer1(Phi_tensors[0])))

        add_layer = Add()
        out_added = add_layer([dense_layer1(Phi_tensors[0]), dense_layer2(dense_layer1(Phi_tensors[0]))])

        d_layer3 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer3')
        dense_layer3 = TimeDistributed(d_layer3, name='d_layer3')
        act_layer3 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer3, act_layer3])
        #Phi_tensors.append(dense_layer3(dense_layer2(dense_layer1(Phi_tensors[0]))))
        #Phi_tensors.append(act_layer3(dense_layer2(dense_layer1(Phi_tensors[0]))))
        Phi_tensors.append(dense_layer3(out_added))
        Phi_tensors.append(act_layer3(out_added))
        #Phi_layers.append(Dropout(0.1, name='drop1'))
        #Phi_tensors.append(Phi_layers[-1](Phi_tensors[-1]))
        
        add_layer2 = Add()
        out_added2 = add_layer2([out_added, dense_layer3(out_added)])

        d_layer4 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer4')
        dense_layer4 = TimeDistributed(d_layer4, name='d_layer4')
        act_layer4 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer4, act_layer4])
        #Phi_tensors.append(dense_layer4(out_added))
        #Phi_tensors.append(act_layer4(out_added))
        Phi_tensors.append(dense_layer4(out_added2))
        Phi_tensors.append(act_layer4(out_added2))
        #Phi_layers.append(Dropout(0.1, name='drop1'))
        #Phi_tensors.append(Phi_layers[-1](Phi_tensors[-1]))

        d_layer5 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer5')
        dense_layer5 = TimeDistributed(d_layer5, name='d_layer5')
        act_layer5 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer5, act_layer5])
        Phi_tensors.append(dense_layer5(dense_layer4(out_added2)))
        Phi_tensors.append(act_layer5(dense_layer4(out_added2)))
       
        add_layer1 = Add()
        out_added1 = add_layer1([out_added2, dense_layer5(dense_layer4(out_added2))])
        
        d_layer6 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer6')
        dense_layer6 = TimeDistributed(d_layer6, name='d_layer6')
        act_layer6 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer6, act_layer6])
        Phi_tensors.append(dense_layer6(out_added1))
        Phi_tensors.append(act_layer6(out_added1))
        #Phi_layers.append(Dropout(0.2, name='drop3'))
        #Phi_tensors.append(Phi_layers[-1](Phi_tensors[-1]))
        
        d_layer7 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer7')
        dense_layer7 = TimeDistributed(d_layer7, name='d_layer7')
        act_layer7  = _get_act_layer('relu')
        Phi_layers.extend([dense_layer7, act_layer7])
        Phi_tensors.append(dense_layer7(dense_layer6(out_added1)))
        Phi_tensors.append(act_layer7(dense_layer6(out_added1)))
        #Phi_layers.append(Dropout(0.3, name='drop3'))
        #Phi_tensors.append(Phi_layers[1](Phi_tensors[-1]))
        add_layer2 = Add()
        out_added2 = add_layer2([out_added1, dense_layer7(dense_layer6(out_added1))])
        d_layer8 = Dense(150, kernel_initializer='he_uniform', name='Phi_layer8')
        dense_layer8  = TimeDistributed(d_layer8, name='d_layer8')
        act_layer8 = _get_act_layer('relu')
        Phi_layers.extend([dense_layer8, act_layer8])
        Phi_tensors.append(dense_layer8(out_added2))
        Phi_tensors.append(act_layer8(out_added2))
        #Phi_layers.append(Dropout(0.4, name='drop4'))
        #Phi_tensors.append(Phi_layers[1](Phi_tensors[-1]))
        '''

        #Phi_tensors = Phi_tensors[1:]
    ...
    def _construct_F(self):
        ...
        # determine begin inds
        layer_inds, tensor_inds = [len(self.layers)], [len(self.tensors)]
        F_layers, F_tensors = [], [self.tensors[-1]]
        # construct F
        tmp_output1 = self._tensors[-1]
        dense_layer1 = Dense(95, kernel_initializer='he_uniform', name='F_layer1')
        act_layer1 = _get_act_layer('relu')
        F_layers.extend([dense_layer1, act_layer1])
        F_tensors.append(dense_layer1(F_tensors[0]))
        F_tensors.append(act_layer1(F_tensors[0]))

        #add_layer1 = Add()
        #out_added1 = add_layer1([tmp_output1, dense_layer1(F_tensors[-1])])

        dense_layer2 = Dense(95, kernel_initializer='he_uniform', name='F_layer2')
        act_layer2 = _get_act_layer('relu')
        F_layers.extend([dense_layer2, act_layer2])
        F_tensors.append(dense_layer2(dense_layer1(F_tensors[0])))
        F_tensors.append(act_layer2(dense_layer1(F_tensors[0])))

        add_layer2 = Add()
        out_added2 = add_layer2([tmp_output1, dense_layer2(dense_layer1(F_tensors[0]))])
        
        dense_layer3 = Dense(95, kernel_initializer='he_uniform', name='F_layer3')
        act_layer3 = _get_act_layer('relu')
        F_layers.extend([dense_layer3, act_layer3])
        #F_tensors.append(dense_layer3(dense_layer2(dense_layer1(F_tensors[0]))))
        #F_tensors.append(act_layer3(dense_layer2(dense_layer1(F_tensors[0]))))
        F_tensors.append(dense_layer3(out_added2))
        F_tensors.append(act_layer3(out_added2))
        
        dense_layer4 = Dense(95, kernel_initializer='he_uniform', name='F_layer4')
        act_layer4 = _get_act_layer('relu')
        F_layers.extend([dense_layer4, act_layer4])
        F_tensors.append(dense_layer4(dense_layer3(out_added2)))
        F_tensors.append(act_layer4(dense_layer3(out_added2)))

        add_layer3 = Add()
        out_added3 = add_layer3([out_added2, dense_layer4(dense_layer3(out_added2))])

        #F_tensors.append(dense_layer4(out_added3))
        #F_tensors.append(act_layer4(out_added3))

        dense_layer5 = Dense(95,  kernel_initializer='he_uniform', name='F_layer5')
        act_layer5 = _get_act_layer('relu')
        F_layers.extend([dense_layer5, act_layer5])
        F_tensors.append(dense_layer5(out_added3))
        F_tensors.append(act_layer5(out_added3))
         
        dense_layer6 = Dense(95, kernel_initializer='he_uniform', name='F_layer6')
        act_layer6 = _get_act_layer('relu')
        F_layers.extend([dense_layer6, act_layer6])
        F_tensors.append(dense_layer6(dense_layer5(out_added3)))
        F_tensors.append(act_layer6(dense_layer5(out_added3)))

        add_layer4 = Add()
        out_added4 = add_layer4([out_added3, dense_layer6(dense_layer5(out_added3))])
        
        
        dense_layer7 = Dense(95, kernel_initializer='he_uniform', name='F_layer7')
        act_layer7 = _get_act_layer('relu')
        F_layers.extend([dense_layer7, act_layer7])
        F_tensors.append(dense_layer7(out_added4))
        F_tensors.append(act_layer7(out_added4))
        F_tensors = F_tensors[1:]
        '''

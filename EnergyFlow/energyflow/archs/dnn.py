...
def construct_dense(input_tensor, sizes,
    ...
    print("FY construct_dense sizes:{}".format(sizes))
    # iterate to make specified layers
    z = zip(sizes, acts, k_inits, dropouts, l2_regs, names)
    for s, act, k_init, dropout, l2_reg, name in z:
        print('FY test loop:')
        print('s:{}, act:{}, k_init:{}, dropout:{}, 12_regs:{}, names:{}'.format(s, act, k_init, dropout, l2_reg, name))
...
class DNN(NNBase):
    ...
    def _construct_model(self):
        ...
        print("FY model's layers 1:{}".format(self._layers))
        ...
        print("FY model's layers 2:{}".format(self._layers))
        add_layer = Add()
        self._layers.extend(add_layer)
        print("FY model's layers 3:{})".format(self._layers))
        self._tensors.extend(add_layer([self._input, self._tensors[-1]]))
        # deeper layers
        ext_layers, ext_tensors = construct_dense(self._tensors[-1], self.dense_sizes,
                                                acts=self.acts, k_inits=self.k_inits,
                                                dropouts=self.dropouts, l2_regs=self.l2_regs,
                                                names=names)
        print("FY model's lyaers 4:{}".format(ext_layers))
        self._layers.extend(ext_layers)
        print("FY model's layers 5:{}".format(self.layers))
        self._tensors.extend(ext_tensors)
        '''
        # deeper layers 2
        add_layer2 = Add()
        self._layers.extend(add_layer2)

        self._tensors.extend(add_layer2([self._tensors[-1], ext_tensors2]))
        ext_layers2, ext_tensors2 = construct_dense(self._tensors[-1], self.dense_sizes,
                                                acts=self.acts, k_inits=self.k_inits,
                                                dropouts=self.dropouts, l2_regs=self.l2_regs,
                                                names=names)  
        self._layers.extend(ext_layers2)
        '''
        # get output layers
